...
public class PlayerController : MonoBehaviour, IPunObservable, IPunInstantiateMagicCallback
{
    ...
    void Start()
    {
        ...
        {
            //if player is not mine, then it should automatically be controlled by photon data
    ...
    void Update()
    {
        ...
        if (!photonView.IsMine)
        {
            ...
            //body.MovePosition(transform.position + this.playerPos * Time.deltaTime * 10);
            
            _animator.SetFloat("Speed", ani);
        }
        else
        {
            //if(fixedJoystick.Horizontal >= .1f)
            //{
            //    horizontal = runSpeed;
            //} else if(fixedJoystick.Horizontal < -.1f)
            //{
            //    horizontal = -runSpeed;
            //}
            //else
    ...
    private void FixedUpdate()
    {
        if (photonView.IsMine)
        {
            ...
            {
                body.AddForce(transform.up * jumpheigt, ForceMode2D.Impulse);
            }
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        else
        {
            ...
            this.ani = (float)stream.ReceiveNext();
        }
    }
    ...
    public void OnPhotonInstantiate(PhotonMessageInfo info)
    {
        ...
        this.playerScale = new Vector3(0.1f, 0.1f, 1);
    }
